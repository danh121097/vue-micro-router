...
<template>
  <div class="page home-page">
    ...
    <div class="history-bar">
      <button :disabled="!canGoBack" :class="{ disabled: !canGoBack }"
        @click="historyBack && historyBack()">← Back</button>
      <span class="history-label">History</span>
      <button :disabled="!canGoForward" :class="{ disabled: !canGoForward }"
        @click="historyForward && historyForward()">Forward →</button>
    </div>

    <div class="actions">
      <button @click="push('settings')">Settings (fade)</button>
      <button @click="push('profile', { userId: 42, username: 'John Doe' })">Profile (props)</button>
    ...
    <div class="serialize-bar">
      ...
      <button class="btn-restore" :disabled="!hasSavedState" @click="restoreState">
    ...
    <div class="info">
      ...
      <ul>
        ...
        <li><code>historyBack()</code> / <code>historyForward()</code> — history navigation</li>
...
<style scoped>
.page { display: flex; flex-direction: column; align-items: center; justify-content: center; height: 100%; gap: 1.25rem; padding: 2rem; }
h1 { font-size: 2.5rem; }
.history-bar { display: flex; align-items: center; gap: 0.75rem; }
.history-label { font-size: 0.75rem; color: #64748b; text-transform: uppercase; letter-spacing: 0.05em; }
.actions { display: flex; gap: 0.75rem; flex-wrap: wrap; justify-content: center; }
.serialize-bar { display: flex; gap: 0.75rem; }
...
button:hover:not(.disabled):not(:disabled) { background: #334155; }
...
.btn-restore:hover:not(:disabled) { background: #064e3b; }
